...
//    for (i = 0; i < sizeof(key);i++) key[i] = i;
    ...
    fseek(file, 0L, SEEK_SET);
    if (fread(key, 1, keybytes, file) != keybytes)
    {
        printf("Unable to read all bytes from file %s\n", fname);
        exit(EXIT_FAILURE);
    }
    fclose(file);
    printf("Length of key file: %lu\n", keybytes);

    // start encrypt in CPU
    start = clock();
    encryptdemo(key, buf, numbytes);
    end = clock();
    printf("CPU Encryption Time used: %f ms\n",  (double)(end - start)*100 / CLOCKS_PER_SEC);
    printf("CPU Encryption Throughput: %f bytes/second\n",  (double)(276982) / ((double)(end - start) / CLOCKS_PER_SEC));
    // write into file
    ...
    printf("CPU Decryption Time used: %f ms\n",  (double)(end - start)*1000/ CLOCKS_PER_SEC);
