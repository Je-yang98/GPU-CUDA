...
// aes client
void ClientEncrypt(uint8_t ClientID, uint8_t *ClientKey, uint8_t KeySeed, char *EncrypFile, uint8_t *EncryptBuf, unsigned long numbytes){
    clock_t start, end;
    
    printf("Client %d Encryption Start...\n", ClientID);
    // Generate key
    uint16_t seed = KeySeed;
    srand(seed);
    for (int i=0; i<sizeof(ClientKey); i++){
        ClientKey[i] = rand() % 256;
        seed = ClientKey[i];
        //printf("Client Key %X\n", ClientKey[i]);
    }
    ...
    //start = clock();
    encryptdemo(ClientKey, EncryptBuf, numbytes);
    //end = clock();
...
void ClientDecrypt(char* DecryptFile, char* OriginFile, uint8_t *key, uint8_t *buf, unsigned long numbytes, unsigned long padding){
    decryptdemo(key, buf, numbytes);
    FILE *file = fopen(DecryptFile, "wb");
    fwrite(buf, 1, numbytes-padding, file);
    fclose(file);
    
    FILE *decrypt, *origin;
    decrypt = fopen(DecryptFile, "r");
    origin = fopen(OriginFile, "r");
    if ((decrypt == NULL) || (origin == NULL)){
        exit(1);
    }
    ...
    while(1){
        ...
        total++;
        //printf("Privacy Total %d Correct %d\n", total, correct);
    }
    float privacy = (float)(correct) / (float)(total);
    printf("Privacy attack %f\n", privacy);
}


void Server(int client, char* DecryptFile, uint8_t key[][32], uint8_t *buf, unsigned long numbytes, unsigned long padding){
    char FileId[client];
    ...
    for (int i=0; i<client; i++){
        sprintf(FileId, "%d", i);
        char *ClientFile = strcat(FileId, Filename);
        
        // decryption
        //start = clock();
        decryptdemo(key[i], buf, numbytes);
        //end = clock();
...
// Client Encryption
    ...
    clock_t start, end;
    start = clock();
    // Serial Client Encryption
    for (int i=0; i<client; i++){
        ClientEncrypt(id[i], key[i], seed[i], ClientFile[i], buf, numbytes);
        printf("\n");
    }
    end = clock();
    printf("GPU Serial Encryption Time used: %f ms\n",  (double)(end - start) * 1000 / CLOCKS_PER_SEC);
    printf("GPU Serial Encryption Throughput: %f bytes/second\n",  (double)(numbytes) / ((double)(end - start) / CLOCKS_PER_SEC));

// Server Decryption 
    start = clock();
    Server(client, argv[4], key, buf, numbytes, padding);
    end = clock();
    printf("GPU Serial Decryption Time used: %f ms\n",  (double)(end - start) * 1000 / CLOCKS_PER_SEC);
    printf("GPU Serial Decryption Throughput: %f bytes/second\n",  (double)(numbytes) / ((double)(end - start) / CLOCKS_PER_SEC));
